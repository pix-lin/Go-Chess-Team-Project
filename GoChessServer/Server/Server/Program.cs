...
//클라이언트와 서버 간 주고받는 메세지의 구조를 정의하는 클래스
...
public class GomokuServer
{
    ...
    public GomokuServer()
    {
        server = new TcpListener(IPAddress.Any, port); //모든 IP에서 접근 가능한 서버 생성
    }
    ...
    public void Start()
    {
        ...
        while(true)
        {
            ...
            if(totalPlayers <= 2) //최대 2명까지만 입장
            {
                ...
                Console.WriteLine($"[서버] 플레이어 {playerID} Join 메세지 전송");
            ...
            {
                client.Close(); //게임 시작된 후 추가 연결 거부
...
public class ClientHandler
{
    ...
    private void ProcessMessage(string message)
    {
        try
        {
            ...
            Console.WriteLine($"받은 메세지 - 타입: {gameMessage.Type}, 위치: ({gameMessage.X}, {gameMessage.Y}), 플레이어: {gameMessage.PlayerID}");

            switch(gameMessage.Type)
            {
                case MessageType.Place: //돌 배치 요청
        ...
        {
            Console.WriteLine($"메세지 처리 중 오류: {ex.Message}");
        }
    }
}